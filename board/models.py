...
class Thread(models.Model):
    ...
    def print_pub_date(self):
        return (self.pub_date + datetime.timedelta(hours=9)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )  # wrong
    ...
    def print_latest_date(self):
        return (self.latest_date + datetime.timedelta(hours=9)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )  # wrong
...
class Response(models.Model):
    ...
    def print_tweet_date(self):
        return (self.tweet_date + datetime.timedelta(hours=9)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )  # wrong
